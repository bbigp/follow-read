...
class CustomRefreshState extends State<CustomRefresh> with TickerProviderStateMixin<CustomRefresh> {
  ...
  bool _handleScrollNotification(ScrollNotification notification) {
    ...
    if (notification is ScrollUpdateNotification) {
      if (_status == RefreshIndicatorStatus.drag || _status == RefreshIndicatorStatus.armed) {
        if (notification.metrics.axisDirection == AxisDirection.down) {
          _dragOffset = _dragOffset! - notification.scrollDelta!; //向下拖动时，scrollDelta 为负值
          _counter.value = _dragOffset ?? 0;
        }
        _checkDragOffset(notification.metrics.viewportDimension);//viewportDimension当前视口的高度
      }
    ...
    } else if (notification is OverscrollNotification) {//用户下拉到列表顶部后继续拉动
      if (_status == RefreshIndicatorStatus.drag || _status == RefreshIndicatorStatus.armed) {
        if (notification.metrics.axisDirection == AxisDirection.down) {
  ...
  bool _handleIndicatorNotification(OverscrollIndicatorNotification notification) {
    ...
    if (_status == RefreshIndicatorStatus.drag) {//用户正在拖动
  ...
  void _checkDragOffset(double containerExtent) {
    ...
    if (_status == RefreshIndicatorStatus.drag && (_dragOffset! >= (containerExtent * _kDragContainerExtentPercentage))) {
                        ...
                        child: height > 40
                          ...
                          builder: (context, child) {
                            return Transform.translate(
                              offset: _offsetAnimation.value * 40, // 将比例转换为实际偏移量
                              child: Container(
                                width: 8,
                                height: 8,
                                child: Container(
